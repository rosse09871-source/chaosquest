...
# 1. Check if the legacy_logger process is still running
if pgrep -f "legacy_logger.py" >/dev/null 2>&1; then
    echo "❌ 실패: 삭제된 로그 파일을 물고 있는 legacy_logger 프로세스가 여전히 실행 중입니다."
    exit 1
fi

# 2. Check if any deleted files are still held open in /var/log
if lsof +L1 2>/dev/null | grep -q "/var/log/app_ghost.log"; then
