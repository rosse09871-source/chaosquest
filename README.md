...
### 1. 54개 실무 인시던트 시나리오 (7대 도메인 · 18대 트랙)
...
* **컨테이너 & 런타임**: Docker 소켓 권한 오류, 데몬 JSON 문법 파괴, 환경변수 누락
* **데이터베이스 & 캐시**: SQLite 교착상태(Deadlock), Redis OOM 캐시 증발 및 바인딩 오류
* **클라우드 & 보안**: SSH 권한 잠금, AWS 자격증명 포맷 오류, Fail2ban IP 차단 복구

### 2. 듀얼 페인 웹 워크스페이스 (Dual-Pane Web IDE)
* **좌측 패널**: 실시간 인시던트 티켓(증상, 심각도, 배점, 목표시간), 3단계 힌트, 실시간 컨테이너 상태를 분석하는 AI 시니어 사수(김수석) SOS 핫라인
